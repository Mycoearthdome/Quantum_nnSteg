...
def generate_embedding_map(bit_count, pixel_count, password, tag="embed", noise_map=None):
    seed = int(hashlib.sha256((password + f"-{tag}").encode()).hexdigest(), 16)
    rng = np.random.default_rng(seed)

    if noise_map is not None:
        flat_noise = noise_map.flatten()
        if flat_noise.shape[0] != pixel_count:
            raise ValueError(f"[ERROR] Noise map size {flat_noise.shape[0]} doesn't match pixel count {pixel_count}")
        weights = flat_noise.astype(np.float64)
        weights = weights / (np.sum(weights) + 1e-10)
        pixel_indices = rng.choice(pixel_count, size=bit_count, replace=True, p=weights)
    else:
        pixel_indices = rng.choice(pixel_count, size=bit_count, replace=True)

    channels = rng.integers(0, 3, size=bit_count)  # R, G, B
    bit_planes = rng.integers(0, 3, size=bit_count)  # LSB to 3rd bit
...
def embed_lsb_rgb_random(image, bits, password, noise_map=None, permute=True, tag="embed"):
    ...
    print(f"[DEBUG] Image shape: {pixels.shape}, flat_pixels: {flat_pixels.shape}")
    if noise_map is not None:
        print(f"[DEBUG] Noise map shape: {noise_map.shape}")
    else:
        print("[DEBUG] No noise map provided; using uniform embedding distribution.")
    ...
    pixel_indices, channels, bit_planes = generate_embedding_map(len(bits), len(flat_pixels), password, tag=tag, noise_map=noise_map)

    print(f"[EMBED-DEBUG] First 10 pixel indices: {pixel_indices[:10]}")
    print(f"[EMBED-DEBUG] First 10 channels: {channels[:10]}")
    print(f"[EMBED-DEBUG] First 10 bit_planes: {bit_planes[:10]}")
...
def embed_lsb_rgb_separate_header(image, full_data_bytes, password, noise_map=None):
    ...
    header_pixel_indices, _, _ = generate_embedding_map(64, len(flat_pixels), password, tag="header")

    flat_noise = noise_map.flatten()
    flat_noise[header_pixel_indices] = 0.0
    flat_noise /= (flat_noise.sum() + 1e-10)  # re-normalize
    noise_map = flat_noise.reshape(noise_map.shape)

    payload_pixel_indices, _, _ = generate_embedding_map(len(payload_bits), len(flat_pixels), password, tag="payload", noise_map=noise_map)

    overlap = set(header_pixel_indices) & set(payload_pixel_indices)
    if overlap:
        print(f"[WARN] {len(overlap)} pixel indices were reused between header and payload (should be 0).")
    else:
        print("[DEBUG] No pixel overlap between header and payload.")
...
def extract_payload_bits(image, password, total_bits, noise_map=None):
    ...
    pixel_indices, channels, bit_planes = generate_embedding_map(total_bits, pixel_count, password, tag="payload", noise_map=noise_map)  # must be None on extraction!

    print(f"[EXTRACT-DEBUG] First 10 pixel indices: {pixel_indices[:10]}")
    print(f"[EXTRACT-DEBUG] First 10 channels: {channels[:10]}")
    print(f"[EXTRACT-DEBUG] First 10 bit_planes: {bit_planes[:10]}")
...
def extract_lsb_rgb_random(image, password, noise_map=None):
    ...
    payload_bits = extract_payload_bits(image, password, decoded_length, noise_map=noise_map)
...
def embed_file_in_image(image, file_path, password, noise_map=None):
    with open(file_path, "rb") as f:
        file_bytes = f.read()
    ...
    stego_img = embed_lsb_rgb_separate_header(image, final_bytes, password, noise_map=noise_map)
    print("[DEBUG] File data embedded into image.")
    return stego_img

def extract_file_from_image(image, password, noise_map=None):
    ...
    print(f"[DEBUG] UNMasked header bits: {masked_header_bits}")
    unmasked_header_bits = masked_header_bits

    # Step 2: Parse the length and CRC from the header
    length_bits = unmasked_header_bits[:32]
    crc_bits = unmasked_header_bits[32:]
    decoded_length = bits_to_int(length_bits)

    print(f"[DEBUG] Decoded length from header: {decoded_length}")

    if decoded_length <= 0 or decoded_length > image.width * image.height * 9:
        print("[ERROR] Invalid decoded length")
        return None

    print(f"[DEBUG] Decoded bit length from header: {decoded_length}")

    # --- Step 4: Exclude header pixels from noise map ---
    if noise_map is not None:
        pixels = np.array(image, dtype=np.uint8)
        flat_pixels = pixels.reshape(-1, 4)
        header_pixel_indices, _, _ = generate_embedding_map(64, len(flat_pixels), password, tag="header")
        flat_noise = noise_map.flatten()
        flat_noise[header_pixel_indices] = 0.0
        flat_noise /= (flat_noise.sum() + 1e-10)
        noise_map = flat_noise.reshape(noise_map.shape)

    # Step 5: Re-extract full masked bitstream (header + payload)
    total_bit_length = 64 + decoded_length
    all_masked_bits = extract_lsb_rgb_random(image, password, noise_map=noise_map)
    all_masked_bits = all_masked_bits[:total_bit_length]

    # Step 6: Generate quantum key
    full_key_bits = generate_bits_fast(total_bit_length, password)

    # Step 7: Unmask the payload
...
def main():
    ...
    noise_map = compute_noise_map(cover)

    file_to_hide = "secret.txt"
    stego_image = embed_file_in_image(cover, file_to_hide, PASSWORD, noise_map)
    stego_image.save("stego_output.png")

    extracted_content = extract_file_from_image(stego_image, PASSWORD, noise_map)
