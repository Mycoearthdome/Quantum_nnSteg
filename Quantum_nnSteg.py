...
def train_offline_model(max_epochs=3000, save_every=100):
    ...
    def loss(weights):
        ...
        for r, g, b, bit in training_data:
            ...
            combined.compose(qc2, qubits=range(qc1.num_qubits), inplace=True)

            transpiled = transpile(combined, backend_sim)
            result = execute(transpiled, backend_sim, shots=shots, noise_model=noise_model).result()
    ...
    optimizer = SPSA(maxiter=max_epochs)
    ...
    for epoch in range(1, max_epochs + 1):
        try:
            new_weights, loss_val, _ = optimizer.optimize(len(init_point), loss, initial_point=best_weights)
...
def main():
    ...
    cover_img = Image.open("bird.jpeg").convert("RGBA")
    ...
    backend = service.least_busy(
        backend_filter=lambda b: b.num_qubits >= 5 and not b.simulator and b.operational
    )
